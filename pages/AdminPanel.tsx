...
export const AdminPanel: React.FC<AdminPanelProps> = ({ currentUser }) => {
  ...
  const isSuperAdmin = (currentUser.username || '').toLowerCase() === 'faisalhassan';
  ...
  const handleOpenCreate = () => {
    if (!isSuperAdmin) {
        alert("Access Denied: Only the Super Admin (faisalhassan) can create new users.");
        return;
    }
    setEditingUser(null);
    setUsername('');
  ...
  const handleDelete = async (userId: string) => {
    if (!isSuperAdmin) {
      alert("Access Denied: Only the Super Admin (faisalhassan) can delete users.");
      return;
    }
    if (window.confirm('Are you sure you want to delete this user? This will also remove their messages and assigned tasks.')) {
      try {
  ...
  const handleSave = async () => {
    ...
    try {
        if (editingUser) {
          ...
          // Double check on save just in case
          if (!isSuperAdmin) return alert("Access Denied: Cannot create user.");
        ...
        // Check for Postgres check violation (code 23514)
              ...
              <div className="flex">
                  ...
                  <div className="ml-3">
                      <p className="text-sm text-yellow-700">
                          ...
                          Creating users, deleting users, and modifying roles are restricted to the Super Admin (<strong>faisalhassan</strong>).
                      </p>
                  </div>
              </div>
          </div>
            ...
            {users.map(user => (
              <tr key={user.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                  ...
                  {isSuperAdmin && user.username.toLowerCase() !== 'faisalhassan' && ( 
                    <button 
      ...
      >
        <div className="space-y-4">
          ...
          <Input 
            ...
            disabled={editingUser?.username.toLowerCase() === 'faisalhassan'}
          />
          <Input 
            label="Password" 
            value={password} 
            ...
            <div className={`flex flex-col space-y-2 mt-2 ${!isSuperAdmin ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
              <label className="flex items-center space-x-2 cursor-pointer p-2 border rounded-md hover:bg-gray-50">
                <input 
                  ...
                  disabled={!isSuperAdmin || editingUser?.username.toLowerCase() === 'faisalhassan'} 
